...
class SRFlowModel(BaseModel):
    ...
    def get_sr_with_z(self, lq, heat=None, seed=None, z=None, epses=None):
        ...
        z1 = self.get_z(heat, seed, batch_size=lq1.shape[0], lr_shape=lq1.shape) if z is None and epses is None else z
        z2 = self.get_z(heat, seed, batch_size=lq2.shape[0], lr_shape=lq2.shape) if z is None and epses is None else z
        z3 = self.get_z(heat, seed, batch_size=lq3.shape[0], lr_shape=lq3.shape) if z is None and epses is None else z
        z4 = self.get_z(heat, seed, batch_size=lq4.shape[0], lr_shape=lq4.shape) if z is None and epses is None else z

        with torch.no_grad():
            sr1, logdet = self.netG(lr=lq1, z=z1, eps_std=heat, reverse=True, epses=epses)
            sr2, logdet = self.netG(lr=lq2, z=z2, eps_std=heat, reverse=True, epses=epses)
            sr3, logdet = self.netG(lr=lq3, z=z3, eps_std=heat, reverse=True, epses=epses)
            sr4, logdet = self.netG(lr=lq4, z=z4, eps_std=heat, reverse=True, epses=epses)
        sr12 = torch.cat((sr1,sr2),2) 
        sr34 = torch.cat((sr3,sr4),2)
        sr = torch.cat((sr12,sr34),3)
        self.netG.train()
        return sr, z
