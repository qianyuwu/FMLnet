...
class RRDBNet(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, opt=None):
        ...
        self.performer = SelfAttention(dim = nf, heads = 8, causal = False).cuda()
        #### upsampling
    ...
    def forward(self, x, get_steps=False):
        ...
        w = trunk.shape[2]
        h = trunk.shape[3]
        trunk = rearrange(trunk, 'b c w h -> b (w h) c')  #Transpose 
        trunk = self.performer(trunk).cuda()
 #       p = trunk.shape[1]
        trunk = rearrange(trunk, 'b (p1 p2) c ->b c p1 p2', p1 = w, p2 =h) 
